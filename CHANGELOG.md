...
### Fixed
- **Kraken: ook trades uit de oude TradeJournal gaan op in de nieuwe levensloop-trades.** Kwamen je Kraken-trades via de migratie uit de oude journal (één rij per groep sluitingen, tijden als tekst), dan bleven die bij het inlezen van je account-log naast de nieuwe trades staan: 20 oude + 23 nieuwe = 43. Nu herkent de import elke oudere Kraken-rij van hetzelfde pair en dezelfde richting met een moment binnen de levensloop, en gaat die erin op. Lees je account-log opnieuw in en het aantal zakt naar het echte aantal posities; notities en labels blijven.
...
### Fixed
- **Kraken: oude CSV-trades worden bij opnieuw inlezen automatisch vervangen.** Trades die met de importer van vóór v0.9.142 waren ingelezen (size als coin-hoeveelheid in plaats van euro, entry met veertien decimalen, UTC-tijden, geen stappen) gaan bij het opnieuw inlezen van je account-log op in de nieuwe levensloop-trade. Je notities, labels, setup en beoordeling verhuizen mee; niets handmatig verwijderen.
...
- **Exporteren op periode.** In het export-overzicht kies je een van- en tot-datum (op de open-datum van de trade); de trades-regel telt mee ("212 van 1.139 trades"), de prullenbak volgt dezelfde periode, de rest van het bestand blijft compleet. Het bestand heet …-periode.json, telt niet als volledige back-up, en bij het importeren staat de periode in de kop met de hint om samen te voegen.
...
### Gewijzigd
- **De P&L per trade staat nu op de cent.** Een winst van $ 0,25 stond als "+$ 0"; nu "+$ 0,25". Geldt voor de tradelijst (ook het geboekte deel van een lopende positie en de niet-gerealiseerde P&L), het totaal onder de lijst en de recente trades op het dashboard.
...
### Fixed
- **De knop "Stappen ophalen" stond soms bij een trade waar niets te halen viel.** Bij Kraken en MEXC leverde hij gegarandeerd "niets op te halen" op. Hij verschijnt nu alleen waar de koppeling de stappen ook echt kan leveren.

### Gewijzigd
- Kraken haalt zijn stappen uit de positie-gebeurtenissen; het ongebruikte fills-pad is eruit. MEXC's fills-pad ook — die koppeling is gestopt en het werd nergens meer voor gebruikt.
...
### Fixed
- **Trades van dezelfde dag stonden in willekeurige volgorde.** De lijst sorteerde alleen op de datum, niet op het tijdstip erbij — dus kon een trade van 17:08 onder één van 04:03 belanden. Hij sorteert nu op het moment van de trade, en bij precies hetzelfde moment blijft de volgorde vast.
...
### Fixed
- **Data uit de oude TradeJournal liep niet door de opruimstappen.** Daardoor bleven posities die in stappen sloten als losse rijen staan — met hun P&L dubbel geteld, want de oude journal bewaarde bij elke sluiting de lopende totaalstand opnieuw. Overzetten doet die stappen nu wél, dus je ziet per positie één trade met het juiste bedrag.
- Het overzicht na het overzetten vertelt nu hoeveel rijen zijn samengevoegd, en de controle achteraf rekent daarmee in plaats van het als fout te melden.
- Een onleesbare regel in je oude opslag liet het overzetten vastlopen. Zulke regels worden nu overgeslagen en benoemd.
...
### Fixed
- Een back-up met kapotte stappen (geen lijst, of lege regels erin) liet het tradeformulier vastlopen. Zulke stappen worden nu bij het laden opgeschoond en de weergave leest ze alleen nog via één veilige route.
- Hyperliquid: dezelfde fill twee keer aangeleverd (bijvoorbeeld door overlappende sync-vensters) telde dubbel mee, waardoor een positie nooit meer vlak leek. Fills worden nu eerst ontdubbeld.
- Hyperliquid: mislukt het terugkijken naar een instap van vóór het sync-venster, dan valt de sync niet meer om; de positie krijgt een afgeleide instap (≈) zoals bedoeld.
...
### Fixed
- **"Geen fills gevonden" kwam er altijd uit bij OKX, wat de exchange ook antwoordde.** De OKX-koppeling had het ophalen van fills per ongeluk twee keer beschreven; de tweede versie won, gaf een ander soort antwoord terug en liet bovendien alleen je afsluitingen door. Je instappen en bijkopen waren dus sowieso weggefilterd. De dubbele versie is weg.

### Bekend
- De proxy meldt nog niet wáárom OKX niets teruggeeft: een ongeldige sleutel en "er zijn geen fills" komen er allebei als leeg uit. De aanpassing daarvoor staat klaar in `docs/worker-okx-complete.js` en moet in Cloudflare worden gezet.
...
### Toegevoegd
- **Markering bij een koppeling die nog niet af is.** Kraken Futures staat in Instellingen → Accounts nu met het label **"in ontwikkeling"**, met daaronder de reden: kijk je trades na een sync even na, want er kunnen trades ontbreken of bedragen afwijken. Je kunt gewoon koppelen — anders dan bij een gestopte API zoals MEXC. Ontdek je iets dat niet klopt, meld het in de Discord.
...
### Gewijzigd
- **Google Drive: één keer koppelen is genoeg** — na het (opnieuw) verbinden blijft de Drive-backup voortaan gewoon werken, ook na uren of dagen; de app verlengt de toegang stil op de achtergrond. Het "Opnieuw verbinden"-scherm verschijnt alleen nog als je de koppeling zelf intrekt bij Google. Je sleutels blijven zoals altijd uitsluitend in je eigen browser en komen nooit in backups terecht.
...
### Fixed
- **OKX: gedeeltelijk gesloten positie toonde TP als 100% i.p.v. het echte percentage** — Sloot je een OKX-positie deels (bv. 5 van 9 contracten op TP1) terwijl de rest open bleef, dan zag de journal die TP als 100% van de positie i.p.v. ~56%, en raakte het open restant zoek. Oorzaak: OKX levert een gedeeltelijke sluiting als apart history-record (met de originele positiegrootte in `openMaxPos`), terwijl de afrond-logica 'm als een volledig afgesloten trade behandelde en de open-placeholder wegruimde. Nu wordt een partial-close herkend, blijft het open restant staan, en klopt het TP-percentage (gesloten ÷ origineel). (Geborgd met [tests/okx-partial-open.spec.js](tests/okx-partial-open.spec.js); cross-exchange-sweep van 90 tests groen.)
