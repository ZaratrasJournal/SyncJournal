/**
 * Morani Trading Journal — Cloudflare Worker Proxy v12
 *
 * Changes vs v11 (alleen Kraken trades-action — entry-price fix):
 *  - v11 debug toonde dat `old_average_entry_price` en `new_average_entry_price`
 *    NULL zijn voor trades. mark_price was niet de echte entry maar dichtbij
 *    exit. Geen account-log veld bevat de open-prijs voor closed positions.
 *
 *  - v12 fix: derive entry uit pnl-math.
 *    realized_pnl is autoritatief (gross, matcht Kraken UI W&V exact):
 *      pnl_gross = (exit - entry) × size × dirSign
 *      entry = exit - (dirSign × pnl_gross / size)
 *    Geverifieerd op sample: 78307 - (-1 × 1.0524 / 0.0012) = 79184 ✓
 *    matcht Kraken UI Openingskoers exact.
 *
 *  - v11 features behouden: tpLevels per close-fill, originalSizeAsset,
...
async function handleKraken(action, { apiKey, apiSecret, startTime }) {
  ...
  const trades = [...groups.values()].map(g => {
    ...
    // v12: derive entry uit pnl-math. realized_pnl is GROSS (= Kraken UI W&V).
    //   pnl_gross = (exit - entry) × size × dirSign
    //   entry = exit - dirSign × pnl_gross / size
    // Voor short (dirSign=-1) + winst: entry > exit (gain als prijs daalt) ✓
    // Voor long (dirSign=+1) + winst: entry < exit (gain als prijs stijgt) ✓
    const dirSign = g.direction === 'long' ? 1 : -1;
    const entryPrice = totalSize > 0 && avgExit > 0
      ? avgExit - (dirSign * totalPnl / totalSize)
      : avgExit;
  ...
  if (trades.length === 0) {
    ...
    return { source: 'account_log_fallback', trades: fallbackTrades, _v12Debug: debug };
  }

  return { source: 'fills+account_log', trades, _v12Debug: debug };
}
