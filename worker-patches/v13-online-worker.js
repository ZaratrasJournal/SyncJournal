/**
 * Morani Trading Journal — Cloudflare Worker Proxy v13
 *
 * Changes vs v12 (alleen Kraken trades-action — partial-close grouping fix):
 *  - v12 werkte voor single-close trades, maar partial-closes (positie in
 *    meerdere chunks gesloten over meerdere minuten) werden als losse trades
 *    geïmporteerd. Bv. snapshot toonde 5 fragmenten @70721 die eigenlijk
 *    1 trade waren.
 *
 *  - v13 wijzigingen:
 *    1. Per-fill entry derivation: afgeleid uit pnl-math per individuele fill
 *       (was per-groep in v12). Voor partial-closes van zelfde positie zijn
 *       alle entry_per_fill identiek (= avg entry van de positie).
 *    2. Group-key: (symbol|direction|entry_rounded_2dec) ipv minute-based.
 *       Fills met identieke entry = zelfde positie, ongeacht tijdvenster.
 *       Edge case (separate posities met identieke entry) accepteren we.
 *    3. realized_funding meegeteld in pnl-berekening.
 *       pnl_net = realized_pnl - fee + realized_funding (matcht new_balance).
 *
 *  - v11/v12 features behouden: tpLevels per close-fill, originalSizeAsset,
 *    firstMatchedEvent diagnostic.
 *
...
async function handleKraken(action, { apiKey, apiSecret, startTime }) {
  ...
  // Stap 5 (v13): per-fill entry derivation + group on (symbol|direction|entry).
  // Was eerder per-minute grouping wat partial-closes brak die over meer dan
  // 1 minuut sluiten. Per-fill entry = trade_price - dirSign × pnl_gross / size.
  // Alle close-fills van dezelfde positie hebben identieke entry_per_fill (=
  // avg entry van de positie) ongeacht tijdvenster.
  ...
  for (const fill of allFills) {
    ...
    // v13: per-fill entry derivation
    const fSize = Math.abs(parseFloat(fill.size || 0));
    const fPrice = parseFloat(fill.price || 0);
    const fPnlGross = parseFloat(event.realized_pnl || 0);
    const dirSign = direction === 'long' ? 1 : -1;
    let entryPerFill = 0;
    if (fSize > 0 && fPrice > 0) {
      entryPerFill = fPrice - (dirSign * fPnlGross / fSize);
    }
    // Round naar 2 decimalen voor robust grouping (small fee/funding variations
    // tussen fills kunnen anders verschillende entries opleveren voor zelfde positie)
    const entryRounded = entryPerFill > 0 ? Math.round(entryPerFill * 100) / 100 : 0;

    // v13 group-key: (contract|direction|entryRounded). Werkt over meerdere minuten.
    const symbolLower = String(fill.symbol).toLowerCase();
    const key = `${symbolLower}|${direction}|${entryRounded}`;
    if (!groups.has(key)) groups.set(key, {
      fills: [], events: [], contract: fill.symbol, direction, firstTs: fillTs,
      derivedEntry: entryPerFill, // bewaar precieze entry van eerste fill
    });
  ...
  const trades = [...groups.values()].map(g => {
    ...
    let totalPnl = 0, totalFee = 0, totalFunding = 0;
    g.events.forEach(e => {
      totalPnl += parseFloat(e.realized_pnl || 0);
      totalFee += Math.abs(parseFloat(e.fee || 0));
      // v13: realized_funding meetellen. Negatief = jij betaalt, positief = jij ontvangt.
      // Behoud teken zodat netto-formule correct is: pnl_net = pnl - fee + funding.
      totalFunding += parseFloat(e.realized_funding || 0);
      if (!firstMatchedEvent) {
        firstMatchedEvent = { ...e };
      }
    });
    const avgExit = totalSize > 0 ? weightedPrice / totalSize : 0;
    // v13: gebruik per-fill afgeleide entry (consistent over partial-close fills
    // van zelfde positie). g.derivedEntry is van de eerste fill in de groep.
    const entryPrice = g.derivedEntry > 0 ? g.derivedEntry : avgExit;
    ...
    return {
      ...
      // v13: pnl_net = realized_pnl - fee + realized_funding. Funding is meestal
      // negatief (jij betaalt) maar kan positief zijn (jij ontvangt). Klopt met
      // new_balance veld in account-log na de close.
      pnl: String((totalPnl - totalFee + totalFunding).toFixed(4)),
      fee: Math.abs(totalFee),
      funding: totalFunding, // v13: apart veld voor display in modal
  ...
  if (trades.length === 0) {
    ...
    return { source: 'account_log_fallback', trades: fallbackTrades, _v13Debug: debug };
  }

  return { source: 'fills+account_log', trades, _v13Debug: debug };
}
