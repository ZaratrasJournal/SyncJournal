/**
 * Morani Trading Journal — Cloudflare Worker Proxy v14
 *
 * Changes vs v13 (alleen Kraken — nieuwe diagnostic action):
 *  - v14 voegt experimentele `position_events` action toe voor Kraken die
 *    /api/history/v3/positions (en alternatieve paden) probeert te hitten.
 *    Doel: zien of dit endpoint closed-position aggregaten retourneert
 *    (= equivalent van CSV historyFuturesClosedPositions). Research
 *    suggereerde dat het endpoint bestaat in de docs-menu maar niet in
 *    SDK's. Dit is een 30-min experiment.
 *
 *  - v13 trades-action ONGEWIJZIGD. Single-close trades blijven werken,
 *    partial-close grouping zoals in v13.
 *
 * v13 features (behouden):
 *  - tpLevels per close-fill
 *  - per-fill entry derivation
 *  - realized_funding in pnl
 *  - originalSizeAsset
 *
 * Actions per exchange: test, trades, open_positions, fills
 *   + Kraken: position_events (experimenteel)
 */
