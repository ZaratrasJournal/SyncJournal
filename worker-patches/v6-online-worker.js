...
async function handleMEXC(action, { apiKey, apiSecret, startTime, symbol, positionId, endTime }) {
  ...
  if (action === 'fills') {
    // v12.93: switch from order_deals → history_orders.
    //
    // Reden: research 2026-05-06 wees uit dat order_deals het position_id query
    // parameter stilzwijgend negeert én structureel alleen open-fills retourneert.
    // Daardoor zag de client consistent 0 close-fills voor closed posities.
    //
    // history_orders heeft per record een `positionId` veld zodat we lokaal kunnen
    // filteren, en levert zowel open- als close-orders met `dealAvgPrice` + `dealVol`
    ...
    // Filter on positionId (history_orders has positionId per record — order_deals didn't)
    const matchingOrders = positionId
      ? allOrders.filter(o => String(o.positionId) === String(positionId))
      : allOrders;
    // Filter only completed orders (state=3) and map to fill-shape consistent
    // with existing client-side parsing (vol/price/profit/fee/side/orderId/timestamp).
