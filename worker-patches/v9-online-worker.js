/**
 * Morani Trading Journal — Cloudflare Worker Proxy v9
 *
 * Changes vs v8 (alleen Kraken trades-action — verdere match-fix):
 *  - v8 leverde fillsCount=328, logsCount=1000, tradeEventsCount=24,
 *    matchedCount=0 — fills + events kwamen binnen, maar tijden lagen
 *    blijkbaar verder dan 30s uit elkaar.
 *
 *  - v9 fixes:
 *    1. Match-window 30s → 5 minuten (account-log settlement delay
 *       t.o.v. fillTime kan groot zijn)
 *    2. Scan ±2 minuten → ±5 minuten
 *    3. firstMatchAttempt bug: vorige check `allFills.indexOf(fill) === 0`
 *       skipte als eerste fill out-of-window was → null. Nu gebruikt
 *       counter zodat het de eerste IN-window fill rapporteert.
 *    4. Extra debug: deltaSamples (delta tussen eerste 3 binnen-window
 *       fills en hun dichtstbijzijnde event), sideCounts (sell vs buy),
 *       inWindowFillsCount, firstInWindowFillSample
 *
...
async function handleKraken(action, { apiKey, apiSecret, startTime }) {
  ...
  if (trades.length === 0) {
    ...
    return { source: 'account_log_fallback', trades: fallbackTrades, _v9Debug: debug };
  }

  return { source: 'fills+account_log', trades, _v9Debug: debug };
}
