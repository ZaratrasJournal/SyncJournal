/**
 * Morani Trading Journal — Cloudflare Worker Proxy v8
 *
 * Changes vs v7 (alleen Kraken trades-action — match-logic fix):
 *  - v7 leverde fillsCount=328, logsCount=1000, matchedCount=0 voor Denny.
 *    De match tussen fills en log-entries faalde voor élke fill.
 *
 *  - v8 fixes:
 *    1. Case-insensitive contract-matching (`pf_xbtusd` vs `PF_XBTUSD`)
 *    2. Match-window 5s → 30s (account-log entries kunnen propagation-
 *       delay hebben t.o.v. fillTime)
 *    3. Scan ±2 minuten i.p.v. ±1 (voor randgevallen rond minute-boundary)
 *    4. Diagnostic fields toegevoegd aan _v8Debug: firstFillSample,
 *       firstLogSample, eerste matchAttempt, en aantal candidate logs per
 *       fill — zodat we kunnen zien WAAR de mismatch zit als 'm faalt
 *
...
async function handleKraken(action, { apiKey, apiSecret, startTime }) {
  ...
  // v8: window 5s → 30s, scan ±2 minuten
  const MATCH_WINDOW_MS = 30 * 1000;
  const findMatchingEvent = (fill) => {
    ...
    for (const m of [minute, minute - 1, minute + 1, minute - 2, minute + 2]) {
      const key = `${symbol}|${m}`;
  ...
  let matchedCount = 0;
  let firstMatchAttempt = null;
  for (const fill of allFills) {
    const fillTs = new Date(fill.fillTime || 0).getTime();
    if (!fillTs || fillTs < since) continue;
    const event = findMatchingEvent(fill);
    if (!firstMatchAttempt && allFills.indexOf(fill) === 0) {
      firstMatchAttempt = {
        fill_symbol: fill.symbol,
        fill_fillTime: fill.fillTime,
        fill_side: fill.side,
        eventLookup_keys_count: eventLookup.size,
        sample_event_keys: [...eventLookup.keys()].slice(0, 3),
        match_found: !!event,
      };
    }
    if (!event) continue;
  ...
  if (trades.length === 0) {
    ...
    return { source: 'account_log_fallback', trades: fallbackTrades, _v8Debug: debug };
  }

  return { source: 'fills+account_log', trades, _v8Debug: debug };
}
