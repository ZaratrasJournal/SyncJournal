...
async function handleKraken(action, { apiKey, apiSecret, startTime }) {
  ...
  // v16: Try/catch rond de hele flow zodat errors als JSON terug komen ipv 500.
  // Geen pagination — 1 call geeft tot 1000 elements (= ruim voor 50+ trades).
  let allElements = [];
  let firstResponseKeys = [];
  let firstResponseBytes = 0;
  let errorTrace = null;
  try {
    const data = await krakenGet('/api/history/v3/positions');
    firstResponseKeys = data ? Object.keys(data) : [];
    firstResponseBytes = JSON.stringify(data || {}).length;
    allElements = (data && Array.isArray(data.elements)) ? data.elements : [];
  } catch (e) {
    errorTrace = String(e?.message || e || 'unknown error').slice(0, 500);
    return {
      source: 'position_updates_error',
      trades: [],
      _v16Debug: { errorTrace, stage: 'fetch_positions', firstResponseKeys, firstResponseBytes },
    };
  }
  ...
  // Elke element heeft .event.PositionUpdate met fields die we nodig hebben.
  // positionChange === 'close' (= partial of full close). 'open' wordt geskipt.
  closeEvents = allElements
    .filter(el => {
      const u = el?.event?.PositionUpdate;
      if (!u) return false;
      if (u.positionChange !== 'close') return false;
    ...
    return {
      source: 'position_updates_error',
      trades: [],
      _v16Debug: { errorTrace: String(e?.message || e).slice(0, 500), stage: 'filter_close_events', totalElements: allElements.length, firstResponseKeys, firstResponseBytes },
    };
  }
