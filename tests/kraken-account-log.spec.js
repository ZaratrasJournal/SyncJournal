...
(async () => {
  ...
  console.log('─── Doorgifte naar de journal: TP-momenten blijven staan ───');
  {
    ...
    // een rij uit de oude journal (gemigreerd): kraken_<fill-id>, ISO-tijden, één rij per groep sluitingen, UTC-sluittijd als datum/tijd
    await p.evaluate(() => { T.push({ id: nextId(), exchange: 'kraken', account: '', srcId: 'kraken_6f1c2a3b-0000-4000-8000-000000000001', pair: 'BTC/USD', dir: 'long', date: '2026-05-01', time: '11:00', entry: 75000, exit: 75500, stop: 0, size: '75', pnl: 0.45, fees: 0.05, r: 0, status: 'closed', kind: 'live', tps: [], layers: [], emotions: [], mistakes: [], checks: [], screenshots: [], tvLinks: [], tags: ['oude-journal'], notes: '', openTime: '2026-05-01T10:00:00.000Z', closeTime: '2026-05-01T11:00:00.000Z' }); persist(); });
    const n3 = await p.evaluate(t => sjImportCsvText(t, ''), csv);
    const na3 = await p.evaluate(() => T.filter(t => t.exchange === 'kraken').map(t => ({ srcId: t.srcId, tags: t.tags })));
    ok('een gemigreerde rij uit de oude journal (ISO-tijden, kraken_<fill-id>) gaat ook op in de levensloop (eerder overgenomen labels blijven)', n3 === 0 && na3.length === 1 && /^kraken_pos_/.test(na3[0].srcId) && (na3[0].tags || []).length > 0, JSON.stringify(na3));
    // en andersom: de nieuwe trade bestond al (eerder ingelezen op v0.9.142/143), de oude rij komt er later bij → opnieuw inlezen ruimt hem alsnog op
    await p.evaluate(() => { T.push({ id: nextId(), exchange: 'kraken', account: '', srcId: 'kraken_csv_BTC/USD|long|2026-05-01|10:00|75000|75550|1.1|0.00200000|x', pair: 'BTC/USD', dir: 'long', date: '2026-05-01', time: '10:00', entry: 75000, exit: 75550, stop: 0, size: '0.002', pnl: 1.1, fees: 0.2, r: 0, status: 'closed', kind: 'live', tps: [], layers: [], emotions: [], mistakes: [], checks: [], screenshots: [], tvLinks: [], tags: [], notes: 'late notitie', openTime: '', closeTime: '' }); persist(); });
    const n2 = await p.evaluate(t => sjImportCsvText(t, ''), csv);
    const na2 = await p.evaluate(() => T.filter(t => t.exchange === 'kraken').map(t => ({ srcId: t.srcId, notes: t.notes })));
    ok('bestond de levensloop al, dan ruimt opnieuw inlezen de oude rij alsnog op (0 nieuw, 1 over, notitie mee)', n2 === 0 && na2.length === 1 && /^kraken_pos_/.test(na2[0].srcId) && !!na2[0].notes, JSON.stringify(na2));
