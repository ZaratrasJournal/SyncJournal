...
(async () => {
  ...
  ok('demo-dataset.json staat naast de app (≥1000 trades, hooguit een week oud, geldige backup-vorm, TradingPlan-blok)', (() => { try { const d = JSON.parse(fs.readFileSync(path.resolve('site', 'demo-dataset.json'), 'utf8')); return d.app === 'SyncJournal' && Array.isArray(d.trades) && d.trades.length >= 1000 && !!d.exMeta && !!d.tradingplan && (Date.now() - +new Date(d.trades[0].date)) < 8 * 864e5; } catch (e) { return false; } })());
