...
(async () => {
  ...
  ok('demo-dataset.json staat naast de app (≥1000 trades t/m vandaag, geldige backup-vorm, TradingPlan-blok)', (() => { try { const d = JSON.parse(fs.readFileSync(path.resolve('site', 'demo-dataset.json'), 'utf8')); return d.app === 'SyncJournal' && Array.isArray(d.trades) && d.trades.length >= 1000 && !!d.exMeta && !!d.tradingplan && d.trades[0].date === new Date().toISOString().slice(0, 10); } catch (e) { return false; } })());
