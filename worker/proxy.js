/**
 * Morani Trading Journal — Cloudflare Worker Proxy v4
 *
 * Zelfde request-shape als v3 — nieuwe implementaties die matchen met de
 * werkende lokale proxy (proxy-local/server.js). Key changes:
 *  - MEXC: position/list/history_positions (geaggregeerde posities)
 *  - Blofin: account/positions-history (position-level, niet order-level)
 *  - Kraken: history/account-log + server-side positie-lifecycle tracking
 *
...
async function handleMEXC(action, { apiKey, apiSecret, startTime, symbol, positionId, endTime }) {
  ...
  const sign = async (qs) => hmacHex(apiSecret, apiKey + ts + qs);
  const headers = async (qs) => ({
    'ApiKey': apiKey,
    'Request-Time': ts,
    'Signature': await sign(qs),
    'Content-Type': 'application/json',
  });
  const get = async (path, params = {}) => {
    const qs = new URLSearchParams(params).toString();
    const res = await fetch(`${base}${path}${qs ? '?' + qs : ''}`, { headers: await headers(qs) });
    const data = await res.json();
    if (!data.success) throw new Error(data.message || `MEXC ${path}`);
    return data;
  };
...
async function handleBlofin(action, { apiKey, apiSecret, passphrase, startTime, endTime, symbol }) {
  ...
  // Signing zoals v3 Worker die werkte: base64(HMAC-SHA256(ts + method + pathWithQs))
  const get = async (path, params = {}) => {
    const qs = new URLSearchParams(params).toString();
...
async function handleKraken(action, { apiKey, apiSecret, startTime }) {
  ...
  // Kraken signing: SHA256(postData + nonce + endpointPath) → HMAC-SHA512 met base64-decoded secret → base64
  ...
  if (action === 'test') {
    ...
    // Probeer een USD balance terug te geven
    let balance = '0';
    try {
  ...
  // trades — account-log met paginatie; groepeer partial fills per minuut (simpel & betrouwbaar)
  const since = startTime ? Number(startTime) : Date.now() - 90 * 86400000;
  const logs = [];
  ...
  // Filter alleen closes met realized PnL (sluiten van positie-events)
  ...
  // Groepeer partial fills per (contract + minuut + richting) tot 1 trade
  const groups = new Map();
  closes.forEach(c => {
    const ts = new Date(c.date || c.dateTime).getTime();
    const minuteKey = Math.floor(ts / 60000);
    const chg = parseFloat(c.change || 0);
    const direction = chg < 0 ? 'long' : 'short'; // size afgenomen = long gesloten
