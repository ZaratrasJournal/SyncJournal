...
- [ ] **Hyperliquid: gesloten trade blijft "open" na auto-sync (handmatige refresh sluit 'm wél)** *(2026-09-03, gemeld door Denny)* — Een op Hyperliquid **gesloten** positie blijft in de journal als **open** staan zolang alleen de **auto-sync** (achtergrond-poll) draait. Pas bij een **handmatige refresh** wordt de trade correct als gesloten afgerond. Symptoom: de open-placeholder wordt op de auto-sync-route niet gefinaliseerd/geconsumeerd door de gesloten round-trip, op de manuele route wél.
  ...
  **Te bevestigen vóór fix** (systematic-debugging — eerst bewijs, niet blind patchen):
  - Loopt de auto-sync door hetzelfde finalize-pad als de handmatige refresh? Roept de auto-poll `fetchTrades` + de open-placeholder-finalize (`openIsPlaceholder` / stale-open-consume) aan, of alleen `fetchOpenPositions`?
  - Scope-verschil: haalt de auto-sync misschien **alleen open posities** op (zonder de fills/round-trips die de close leveren), terwijl de manuele refresh de volledige fetch doet?
  - Timing/throttle: wordt de close-fill door de auto-poll gemist door een rate-limit-skip of fills-cap (backoff/throttle uit de sync-reliability-laag), waardoor de round-trip niet binnenkomt?
  - Snapshot-fixture: capture via `?dev=1` een auto-sync-response mét de gesloten positie → offline reproduceren in een test (zie CLAUDE.md "Snapshot-fixture pattern").

  **Locatie code**: `ExchangeAPI.hyperliquid` (fetchTrades / fetchOpenPositions / close-detectie), de finalize-/`openIsPlaceholder`-flow, en het **auto-sync-poll-pad vs. de handmatige refresh-knop** — die twee paden naast elkaar leggen is de kern.

  **Gerelateerd**: spiegelbeeld van de eerder gefixte Hyperliquid refresh-duplicaat (`openIsPlaceholder`, v12.229/v12.235). Daar promoveerde de placeholder ten onrechte; hier wordt 'ie op de auto-route juist **niet** vervangen door de gesloten round-trip.

  **Acceptatie**: een op Hyperliquid gesloten trade wordt óók door de auto-sync als gesloten afgerond (zelfde eindresultaat als handmatige refresh), zonder duplicaat. Reproductie + fix gedekt door een snapshot-test.

- [ ] **OKX: USDC-margined perps i.p.v. USDT** *(2026-06-19, ontdekt door Denny tijdens OKX-setup — EEA-account)* — OKX EEA verhandelt perps in **USDC** (bv. `BTC-USDC-SWAP`), maar onze OKX-integratie neemt op meerdere plekken **USDT** aan. Te fixen / ccy-agnostisch maken:
  - **Balans-check** (Worker `handleOKX` test-action): query is `/api/v5/account/balance?ccy=USDT`. Werkt nu alleen omdat 'ie terugvalt op `totalEq` → toont totaal-equity maar gelabeld als "USDT". Beter: USDC + USDT proberen, of gewoon `totalEq` gebruiken (USD-waarde).
  - **ctVal-fallbackmap** (`ExchangeAPI.okx._ctvFallback` in work): alleen `*-USDT-SWAP`-keys. USDC-pairs missen → vallen terug op live `/public/instruments` (dekt het meestal) maar fallback-map zou ook USDC moeten hebben.
  - **Default-pair / labels**: check op hardcoded `USDT`-aannames in `_normalise` (pair-mapping `instId.replace('-','/')` werkt al ccy-agnostisch — `BTC-USDC-SWAP` → `BTC/USDC`, dus dat deel is oké).
  - **NB:** `instType=SWAP` in de trades-query is wél ccy-agnostisch (dekt USDT+USDC), dus dat verklaart op zich niet waarom er geen trades kwamen — wordt nu uitgezocht via de snapshot (2026-06-19). Mogelijk een aparte oorzaak; deze backlog-item gaat puur over de USDT-aannames.

- [ ] **Kraken: open positie mogelijk gedupliceerd bij elke refresh (fix geleverd in v12.235 — bevestiging member nodig)** — *Update 2026-06-12: jordy's identieke Hyperliquid-melding is geroot-caused: de stale-open finalize consumeerde de echte round-trip uit fetchTrades en promoveerde de open-placeholder (sync-tijdstip als time, geaggregeerde size); de volgende fetch bracht de round-trip opnieuw binnen → permanent duplicaat. v12.235 introduceert `openIsPlaceholder` op de Hyperliquid- én Kraken-adapter: placeholder vervalt, fills-trade (stabiel id) wint, user-velden verhuizen mee. Zie [tests/hyperliquid-refresh-dup.spec.js](tests/hyperliquid-refresh-dup.spec.js). De 1970-timestamp-bug hieronder blijft apart staan; vraag de Kraken-member na de update om te bevestigen dat de refresh-duplicaten weg zijn.* *(2026-06-07, gemeld door member via Denny)* — Zelfde symptoom als de Hyperliquid-bug (gefixt in v12.229): dezelfde open positie verschijnt na elke handmatige refresh als nieuwe trade. **Verschil**: Kraken's open-ID is in v12.101 bewust stabiel gemaakt (`open_${symbol}_${fillTime||"0"}_${side}`, [work/tradejournal.html:3575](work/tradejournal.html#L3575)), dus dit kan alleen optreden als **`fillTime` per refresh verandert** — wat direct raakt aan de bug hieronder (1970-timestamps / API-format-shift). **Te bevestigen vóór fix** (systematic-debugging Iron Law): (a) hebben de dubbele Kraken-trades verschillende tijdstippen? (b) Kraken-snapshot via `?dev=1` → check of `/openpositions` een stabiele `fillTime` levert. **Fix-richting indien bevestigd**: open-ID losmaken van `fillTime` → `open_${symbol}_${side}` (net-mode = 1 open positie per symbool+richting, zoals Hyperliquid's coin-id). Niet blind patchen — eerst bewijs.
...
- [x] **Trades-pagina toont andere PnL/WR dan Dashboard** ✅ *Afgesloten 2026-06-11: hertest met blofin-partial fixture — Dashboard en Trades tonen identiek $-11,63 / WR 28.6%, beide in $. Opgelost door tussenliggende releases (valuta) + audit v12.233 (alle PnL via netPnl, gedeelde aggregaat-helpers). Win-definitie gedocumenteerd: PARTIAL heeft pnl="" en telt in beide stats niet mee tot volledige sluiting; Trades-header "N trades" is bewust de lijst-count (incl. open/partial), Dashboard de closed-count. Geborgd met [tests/pnl-consistency.spec.js](tests/pnl-consistency.spec.js).* *(2026-05-02, gevonden tijdens pro-trader review — zie [docs/pro-trader-review-2026-05-02.md](docs/pro-trader-review-2026-05-02.md) §2.2)* — Met blofin-partial-state.json fixture toont Dashboard "Net P&L -€8,37 · WR 33,3% · 15 trades" (correct) maar Trades-pagina header-stat-line toont "**-$11,43 · WR 27% · 15 trades**". Drie inconsistencies tegelijk: (a) valuta $ vs €, (b) bedrag −11,43 vs −8,37, (c) WR 27% vs 33,3%. Vermoedelijke oorzaak: PARTIAL-trades verschillend geclassificeerd (Trades-pagina telt 4/15 wins, Dashboard 5/15 — verschil = de 1 PARTIAL trade) + valuta-conversie niet consistent toegepast. **Critical** — pro-trader-vertrouwen breekt bij elke discrepantie. Fix: route alle PnL-berekeningen via `netPnl()` helper (regel 1133), één canonieke valuta-formatter via `config.currency`, één win-definitie (incl. of excl. PARTIAL — kies en documenteer).

- [x] **Floating-point precision in trade-detail modal** ✅ *Opgelost: v12.210 normaliseert bij save; v12.233 (audit fase 3) ook bij draft-restore.* *(2026-05-02, gevonden tijdens pro-trader review — zie [docs/pro-trader-review-2026-05-02.md](docs/pro-trader-review-2026-05-02.md) §2.9)* — Trade Edit-modal toont rauwe float-values: Entry `2255.5805555555557` (16 decimalen), PnL `-6.749084190000000001`. Op Trades-tabel staat de prijs wel afgekapt naar `2.355,58` — dus probleem zit alleen bij display in modal. **Critical** — pro-trader die deze ruis ziet vertrouwt de hele app niet meer. Fix: gebruik bestaande `normPrice()` helper (regel 1149) bij modal-render, of formatter met instrument-relevante precisie (BTC: 2 decimalen, ETH: 2, alts: 4-6).
...
- [x] **MEXC share-card: percentage-toggle laat PnL verdwijnen** ✅ *Afgesloten 2026-06-11: de gemelde MEXC-stijl kaart bestaat niet meer — share-card volledig vervangen in v12.64 (4 directions + per-veld toggles). Geverifieerd dat de nieuwe kaarten het gemelde gedrag niet hebben: %-toggle aan → PnL blijft zichtbaar; % onberekenbaar (geen size) → "—" i.p.v. lege kaart. Geborgd met [tests/share-card-pct.spec.js](tests/share-card-pct.spec.js).* *(2026-04-30, gemeld door NielsB in Discord)* — Op het MEXC share-kaart-paneel toont de PnL-waarde correct het bedrag, maar zodra de "percentage"-checkbox wordt aangevinkt, verdwijnt het PnL-cijfer in plaats van als % weergegeven te worden. Reproductie: trade share-modal openen → MEXC-stijl kaart → toggle "Percentage" aan → PnL-veld blank. Vermoedelijk een rendering-conditie die `pnl > 0` checkt zonder rekening te houden met de percentage-modus. Code: zoek in [work/tradejournal.html](work/tradejournal.html) naar share-card percentage-toggle / MexcShareCard / `fields.percentage`.

- [x] **Hyperliquid live balance wordt niet opgehaald in BALANS bovenin** ✅ *Opgelost in v12.231: balans toont nu volledige portfolio (spot + perps); wallet-only accounts tellen volwaardig mee.* *(2026-04-24, gemeld door Denny)* — Hyperliquid `testConnection` haalt nu alleen `clearinghouseState.marginSummary.accountValue` op (perp-only equity). Te checken: (a) returnt de call überhaupt `success: true` met `balance > 0` voor Denny's wallet — debug via console of de hook hyperliquid wel oppakt; (b) `useLiveExchangeBalances` skipt mogelijk wallet-only exchanges door de check `if(ex.walletOnly){if(!cfg.walletAddress)continue;}` — of werkt die correct?; (c) als de wallet alleen spot-balance heeft (`spotClearinghouseState.balances` met USDC/USDT) telt dat nu niet mee in `accountValue` — uitbreiden met spot-fetch zou nodig zijn (zoals we eerder probeerden in een teruggedraaide v12.30 patch). Reproductie: Denny's profiel met Hyperliquid wallet ingevuld → Hyperliquid contributeert niets aan BALANS bovenin. Code: [work/tradejournal.html:1642-1647](work/tradejournal.html#L1642-L1647) (testConnection) + [work/tradejournal.html:1924-1958](work/tradejournal.html#L1924-L1958) (hook).

## 🔍 Uit de code-audit v12.233 (zie REVIEW.md voor context)

- [ ] **Trades-tabel paginatie of virtualisatie** *(2026-06-11, audit fase 4)* — Gemeten met 1000 trades: tab-switch naar Trades kost 1.4–2.1s omdat alle rijen renderen. Grootste gemeten perf-winst. **UI-keuze → demo-first met Denny** (bv. "Toon meer"-knop per 100 vs infinite scroll vs virtualisatie). Baseline-meting: `node tests/perf-measure.js`. Effort: M-L.
- [ ] **Dashboard: charts lazy mounten na first paint** *(2026-06-11, audit fase 4)* — Dashboard-switch 1.3–1.4s bij 1000 trades (charts + KPI-aggregaten mounten in één commit). Effort: M.
- [ ] **`ftmoCtx` doorgeven aan álle `calcRMultiple`-callers** *(2026-06-11, audit fase 4)* — Trades-tabel gebruikt de mediaan-loser-SL fallback, maar avgR-metric (goals) en playbook-stats vallen op de 0.4%-default → FTMO R-waardes verschillen per scherm. Bewuste gedragswijziging, daarom niet in de audit gedaan. Effort: S.
...
- [ ] **SyncJournal hosted op eigen domein — data lokaal, opt-in cloud-backup** *(2026-09-15, idee Denny: "domein aanvragen waar mensen gebruik kunnen maken via een URL, maar data lokaal of in een vorm van Google Drive")* — Leden gebruiken SyncJournal via een URL i.p.v. download+localhost; localStorage/IndexedDB blijft de opslag (privacy-verhaal intact: wij zien/bewaren niets). Technisch triviaal: single-file → Cloudflare Pages (gratis, zelfde account als de Worker) + domein ~€10–15/jr; oude app heeft al `IS_HOSTED`-detectie. **Twee harde aandachtspunten**: (1) de origin ís de data — domein één keer kiezen en nóóit wisselen (ook www vs apex is een andere origin); (2) browsers mogen lokale data wissen — Safari wist site-data na 7 dagen niet-bezoeken, alle browsers evicten bij schijfdruk → `navigator.storage.persist()` + zichtbare "laatste backup"-indicator + backup-reminder zijn **voorwaarde**, geen luxe. **Fasering**: ① domein + hosting + persist() + backup-indicator → ② auto-backup versterken → ③ opt-in cloud-backup naar eigen Drive/Dropbox van het lid (client-side OAuth, geen backend; wél Google-verificatie + 1-uurs-tokens — daarom opt-in laag, geen fundament; het is backup/restore, géén live multi-device-sync) → ④ later evt. accounts + sync-backend (dan ook API-keys uit de browser). Besproken in sessie 2026-09-15; **volledig gefaseerd plan (incl. backup-keuze lokaal/Drive + 3-dagen-alarm) staat in [docs/syncjournal-hosting-backup-plan.md](docs/syncjournal-hosting-backup-plan.md)** (2026-09-16). Wacht op go per fase.
