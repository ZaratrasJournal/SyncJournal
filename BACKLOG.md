...
- [ ] **Fees worden niet visueel verrekend in PnL bij quick-log (geen entry/exit/size)** *(2026-04-28, gemeld door Denny)* — vervolg op v12.49 fix. Reproductie: + Nieuwe Trade → vul ALLEEN PnL ($5000) en Fees ($50) in zonder entry/exit/positionSize → save → in Trades-lijst toont kolom PnL **+$5.000,00** in plaats van **+$4.950,00**. Zelfde issue in Dashboard totals + Analytics aggregations. **Waarom v12.49 dit niet ving**: de auto-recalc useEffect vereist `entry > 0 && exit > 0 && size > 0` — bij quick-log is dat allemaal leeg, dus geen recalc. Bovendien: zodra user PNL handmatig typt komt `"pnl"` in `manualOverrides` → auto-update blokkeert sowieso bij latere fee-wijziging. **Twee fix-richtingen, kies één**: (A) **Display-fix**: nieuwe helper `netPnl(t)` die voor `source==="manual"` returnt `pnl - fees`, voor exchange-bronnen (blofin/mexc/kraken/hyperliquid) returnt `pnl` (die zijn al netto). Vervang alle `parseFloat(t.pnl)` in de UI/aggregations. Risico: groot refactor, ~30 plekken in code, kans op missen van eentje. (B) **TradeForm-knop**: toon naast PNL+fees rij een "⚖️ Trek fees af → $4950"-knop wanneer pnl+fees beide >0 en `pnl !== pnl-fees`. Click → setTrade pnl = pnl-fees. Veilig (user-driven), simpel, geen schema-changes, ~10 regels code. **Mijn aanbeveling**: B voor snelle fix + duidelijkere PnL-veld-label *"PNL (netto)"* met inline hint *"Vul netto in, of klik onder om fees automatisch af te trekken"*. A is de "echte" fix maar vereist een schema-decision (is `t.pnl` officieel netto of bruto?). Niet relevant voor exchange-imports — die leveren al netto pnl.
