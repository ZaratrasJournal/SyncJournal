...
- [ ] **Fees worden niet verrekend in PnL bij handmatig logged trades** *(2026-04-28, gemeld door Morani via Discord)* — bij handmatig invoeren van een trade (geen API-import) zou de fee van de PnL afgetrokken moeten worden. Volgens Morani: *"vul een fee in en het rekent niet door dat het van de pnl af gaat"*. Sebas heeft hieraan gesleuteld maar het werkt nog niet correct. Reproductie: + Trade → Closed status → vul entry/exit/positionSize/fee in → PnL toont bruto i.p.v. netto (fee niet afgetrokken). Te checken: (a) in TradeForm de PnL-auto-berekening (kijk naar useEffect die pnl berekent uit entry/exit/size — wordt fee daar betrokken?); (b) verschilt het tussen `pnl` veld dat user invult vs. auto-berekend; (c) Analytics/Dashboard — gebruiken die `pnl` of `pnl - fees`? Voor API-trades komt PnL al netto binnen van de exchange, maar voor manual moet TradeJournal zelf rekenen. Mogelijke fix: óf auto-berekening fixen, óf duidelijke "Netto / Bruto"-toggle in TradeForm met tooltip-uitleg. Niet relevant voor exchange-imports (Blofin/MEXC/Kraken returnen `realizedPnl` already-net).
- [ ] **Setup-lagen tag-pills onleesbaar op Parchment-theme** *(2026-04-28, gemeld door Denny)* — bij Trades → trade-detail → "Setup & Psychologie" → "+ Laag toevoegen" worden de tag-pills (Timeframe, Setup, Confirmaties) gerenderd met een donkere/grijze achtergrond + lichte tekst, wat correct is op dark themes maar onleesbaar is op `parchment` (cream-bg light theme). Screenshot toont een grijs blok waar BOS / MSB / SFP / F2R / F2L / Liquidity Sweep / OB / FVG / Divergence / EQL/EQH / ChoCh tags vrijwel onzichtbaar zijn. Te checken: (a) de inline-style of CSS-class die deze pills rendert in `EMPTY_LAYER`-builder — gebruikt 'ie hardcoded `rgba(255,255,255,...)` of `var(--bg3)` die op light themes niet werkt? (b) `parchment` heeft warme cream tinten — pills moeten een lichte border + donkere tekst krijgen op deze theme; (c) check of dezelfde pills ook stuk zijn op `light` en `daylight` themes (CLAUDE.md regel: alle 6 themes testen). Reproductie: switch naar Parchment theme → open een trade → expand "Setup & Psychologie" → klik "+ Laag toevoegen" → pills onder TIMEFRAME / SETUP / CONFIRMATIE(S) zijn nauwelijks zichtbaar. Mogelijke fix: vervang hardcoded pill-colors door `var(--bg4)` background + `var(--text2)` foreground OF voeg per-thema overrides toe in `<style>` block voor `body.theme-parchment .layer-pill`.
- [ ] **"Indeling aanpassen" toggles onzichtbaar in OFF-state op light themes** *(2026-04-28, gemeld door Denny)* — in Analytics → tandwiel-icoon → "Indeling aanpassen" panel zie je per layout-key een toggle (Proces KPI's / Risk consistency / Discipline heatmap / Edge Gap / Stress-Leak / Rolling 20-trade edge / Setup insights / Setup edge / etc). Items die AAN staan tonen een goud-getinte toggle rechts, maar items die UIT staan tonen **geen zichtbare toggle** op `parchment` theme — gebruiker ziet alleen de grijze label-tekst en kan de optie niet weer aanzetten zonder te raden waar te klikken. UX-blocker: gebruiker denkt dat de optie permanent verborgen is. Te checken: (a) toggle-component CSS, off-state heeft waarschijnlijk een te-lichte achtergrond (`rgba(255,255,255,0.05)` of vergelijkbaar) die op cream-bg parchment wegvalt; (b) zelfde issue waarschijnlijk op `light` en `daylight` themes; (c) toggle "off" zou een zichtbare grijze/donkere "rail" moeten hebben — fix met `border:1px solid var(--border3)` + `background:var(--bg3)` werkt op alle 6 themes. Reproductie: Parchment theme → Analytics → ⚙ icoon → scroll door layout-toggles → items met "Proces KPI's", "Discipline heatmap", "Edge Gap", "Rolling 20-trade edge" tonen geen toggle-knop terwijl ze juist UIT staan. Hoge prio: blokkeert dat gebruiker layout zelf kan aanpassen.
